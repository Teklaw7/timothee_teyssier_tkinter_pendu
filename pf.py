# -*- coding: utf-8 -*-
"""
Created on Thu Dec 10 11:30:47 2020
...
liste_mots = fichier.readlines()    # met tous les mots du fichier liste_mots.txt dans une liste
fichier.close()
...
def lettre_dans_mot(lettre) :
    ...
    if partie_en_cours : 
        ...
        if not lettre_dans_mot :        # lettre est fausse donc on change le dessin
            nb_echecs += 1
            nomFichier = "images/bonhomme"+str(nb_echecs)+".gif"
            photo=PhotoImage(file=nomFichier)
            image_pendu.config(image=photo)
            image_pendu.image=photo
            if nb_echecs == 7:  # si toutes les tentatives ont été utilisé cela met fin à la partie
